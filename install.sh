...
# Prompt user for pgAdmin setup details
container_name=$(prompt_user "Enter the pgAdmin container name" "pgadmin")
network_name=$(prompt_user "Enter the network name" "general")
pgadmin_port=$(prompt_user "Enter the pgAdmin port" "8082")
allow_host=$(prompt_user "Enter the allowed host" "0.0.0.0")
pgadmin_email=$(prompt_user "Enter the pgAdmin admin email" "admin@example.com")
pgadmin_password=$(prompt_user "Enter the pgAdmin admin password" "adminpassword")

# Generate the .env file
echo "Creating .env file for pgAdmin setup..."
cat > .env <<EOL
...
if [ -f "docker-compose.example.yaml" ]; then
    sed -e "s/\${CONTAINER_NAME}/$container_name/g" \
        ...
        -e "s/\${PGADMIN_EMAIL}/$pgadmin_email/g" \
        -e "s/\${PGADMIN_PASSWORD}/$pgadmin_password/g" \
...
echo "Starting Docker Compose with --build for pgAdmin..."
if docker-compose up -d --build; then
    echo "Checking container status..."
    if [ "$(docker inspect -f '{{.State.Running}}' $container_name)" = "true" ]; then
        echo "pgAdmin setup is complete and running. Access it at http://127.0.0.1:$pgadmin_port."
    else
